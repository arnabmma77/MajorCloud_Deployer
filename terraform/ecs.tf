  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = var.openweather_api_key != "" ? [
        {
          name      = "OPENWEATHER_API_KEY"
          valueFrom = aws_secretsmanager_secret.openweather_api_key[0].arn
        }
      ] : []
...
# Secrets Manager secret for OpenWeather API key
resource "aws_secretsmanager_secret" "openweather_api_key" {
  count = var.openweather_api_key != "" ? 1 : 0

  name        = "${local.name_prefix}-openweather-api-key"
  description = "OpenWeather API Key for weather data"

  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-openweather-secret"
  })
}

# Store the OpenWeather API key in Secrets Manager
resource "aws_secretsmanager_secret_version" "openweather_api_key" {
  count = var.openweather_api_key != "" ? 1 : 0

  secret_id     = aws_secretsmanager_secret.openweather_api_key[0].id
